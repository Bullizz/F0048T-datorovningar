...
y       = linspace(-d/2, d/2, 200); % y kant

% För definera en matris där alla komplexa amplituder kan sparas.
Uall    = zeros(200,1000,M);

% Loopa igenom alla moder och beräkna den komplexa amplituden
for m = 1:M
    % Beräkna riktnings vektorerna
    s1      = [sind(theta(m)), cosd(theta(m))]; % Riktnings cosinen för vågen i +y
    s2      = [-sind(theta(m)), cosd(theta(m))]; % Riktnings cosinen för vågen i -y
